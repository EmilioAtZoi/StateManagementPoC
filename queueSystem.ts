import { updateThing } from "./mocks/MockCloud";
...
class QueueSystem {
  ...
  private static async syncUpdateDeviceState(payload: {
    ...
    try {
      const updatedState = { [payload.key]: payload.record };
      await updateThing(payload.id, updatedState); // Use the updateThing function directly
      console.log(
